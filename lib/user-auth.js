...
const db = require('./db');
const { isSecureRequest, validatePassword } = require('./security');
...
function validateRegistration({ username, email, password, displayName }) {
  ...
  return {
    ...
    email: normalized,
    password: validatePassword(password, { username: normalizedUsername, email: normalized, displayName: name }),
    displayName: name,
  };
}
...
const DUMMY_PASSWORD_HASH = hashPassword('timing-only-placeholder-password');
...
function authenticateUser(identifier, password) {
  ...
  const row = db.prepare('SELECT * FROM users WHERE disabled=0 AND (email=? OR username=?)').get(normalized, normalized);
  if (!row) {
...
function verifyUserPassword(userId, password) {
  const row = db.prepare('SELECT password_hash FROM users WHERE id=? AND disabled=0').get(userId);
  ...
  const create = db.transaction(() => {
    ...
    db.prepare(`
      DELETE FROM user_sessions WHERE id IN (
        SELECT id FROM user_sessions WHERE user_id=? ORDER BY id DESC LIMIT -1 OFFSET 9
      )
    `).run(userId);
...
function ensureDefaultAdmin({ username = 'admin', email = 'admin@trendhub.local', password, mustChangePassword = true } = {}) {
  const existing = db.prepare("SELECT * FROM users WHERE role='admin' ORDER BY id LIMIT 1").get();
  if (existing) return publicUser(existing);
  const normalizedUsername = validateUsername(username);
  const normalizedEmail = normalizeEmail(email);
  const safePassword = validatePassword(password, { username: normalizedUsername, email: normalizedEmail });
  const result = db.prepare(`
    INSERT INTO users(username,email,display_name,password_hash,role,must_change_password)
    VALUES(?,?,?,?, 'admin', ?)
  `).run(normalizedUsername, normalizedEmail, '管理员', hashPassword(safePassword), mustChangePassword ? 1 : 0);
  return publicUser(db.prepare('SELECT * FROM users WHERE id=?').get(result.lastInsertRowid));
}
...
function updateUserAccount(userId, input) {
  ...
  if (identityChanged && !verifyPassword(String(input.current_password || ''), current.password_hash)) {
    throw new Error('修改用户名或邮箱需要验证当前密码');
  }

  let passwordHash = current.password_hash;
  if (input.new_password) {
    if (!verifyPassword(String(input.current_password || ''), current.password_hash)) throw new Error('当前密码错误');
    const nextPassword = validatePassword(input.new_password, {
      username,
      email,
      displayName,
    });
    if (verifyPassword(nextPassword, current.password_hash)) throw new Error('新密码不能与当前密码相同');
    passwordHash = hashPassword(nextPassword);
  }
  try {
    db.prepare(`
      UPDATE users SET username=?,email=?,display_name=?,password_hash=?,
        must_change_password=CASE WHEN ? THEN 0 ELSE must_change_password END,
        updated_at=datetime('now','localtime')
      WHERE id=?
    `).run(username, email, displayName, passwordHash, input.new_password ? 1 : 0, userId);
  } catch (e) {
...
function deleteDisabledUser(userId) {
  const id = Number(userId);
  if (!Number.isSafeInteger(id) || id <= 0) throw new Error('用户参数错误');
  const remove = db.transaction(() => {
    const target = db.prepare('SELECT id,username,role,disabled FROM users WHERE id=?').get(id);
    if (!target) throw new Error('用户不存在');
    if (!target.disabled) throw new Error('请先停用该用户，再执行永久删除');
    db.prepare('DELETE FROM user_sessions WHERE user_id=?').run(id);
    db.prepare('DELETE FROM user_favorites WHERE user_id=?').run(id);
    db.prepare('DELETE FROM user_dismissals WHERE user_id=?').run(id);
...
function setSessionCookie(session, request) {
  cookieStore().set(USER_COOKIE, session.token, {
    httpOnly: true,
    sameSite: 'lax',
    secure: request ? isSecureRequest(request) : process.env.COOKIE_SECURE === '1',
    path: '/',
...
function deleteCurrentSession(request) {
