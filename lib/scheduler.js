...
const { runFetchAll, runExploreJob } = require('./jobs');
...
function schedule() {
  ...
  const cronExplore = getSetting('cron_explore', '30 3 * * *');
  ...
  if (cron.validate(cronExplore)) {
    tasks.push(cron.schedule(cronExplore, () => { runExploreJob().catch(() => {}); }));
  }
