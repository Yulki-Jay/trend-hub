...
function buildHtml({ repos, news, papers }) {
    ...
    .map(
      ...
      <tr>
        ...
        <td style="padding:8px 6px;">
          <a href="${r.url}" style="color:#111827;font-weight:600;text-decoration:none;">${r.full_name}</a>
          <div style="color:#6b7280;font-size:13px;margin-top:2px;">${(r.description || '').slice(0, 100)}</div>
          <div style="color:#9ca3af;font-size:12px;margin-top:2px;">${r.language || ''} · ⭐ ${r.stars} · 今日 +${r.today_stars}</div>
        </td>
      </tr>`
    )
    .join('');
  ...
  const newsBlocks = Object.entries(newsByCat)
    .map(
      ([cat, list]) => `
      <h3 style="margin:18px 0 8px;color:#111827;">${CATS[cat] || cat}</h3>
      ${list
        .map(
          (n) => `<div style="margin-bottom:10px;">
            <a href="${n.url}" style="color:#1f2937;text-decoration:none;font-weight:600;">${n.title}</a>
            <div style="color:#9ca3af;font-size:12px;">${n.source}</div>
          </div>`
        )
        .join('')}`
    )
  ...
  const paperBlock = papers && papers.length
    ? `<h2 style="margin:24px 0 8px;color:#111827;">📄 前沿论文</h2>` +
      papers.map((p) => `<div style="margin-bottom:12px;">
        <a href="${p.url}" style="color:#1f2937;text-decoration:none;font-weight:600;">${p.title}</a>
        <div style="color:#9ca3af;font-size:12px;">${p.source} · ${p.category}${p.stars ? ' · ⭐' + p.stars : ''}</div>
      </div>`).join('')
    : '';
